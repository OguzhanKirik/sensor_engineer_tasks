              ...
              pcl::PointCloud<pcl::PointXYZI>::Ptr inputCloud)
{
  ...
  // ----------------------------------------------------
   // RENDER OPTIONS (Note: Obst + Plane = pointCloud!)
    ...
    float filterRes = 0.01f;

  
  if(renderInputCloud){
  	renderPointCloud(viewer,inputCloud,"inputCloud");
  }
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud = pointProcessorI->FilterCloud(inputCloud, filterRes , Eigen::Vector4f (-10.0, -5.0, -2.0, 1), Eigen::Vector4f ( 30.0, 10.0, 2.0, 1));
...
//     std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> clusteringResult = pointProcessorI->Clustering(segmentCloud.second , 0.5, 10, 500);  
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> clusteringResult = pointProcessorI->euclideanClustering(segmentCloud.second , 0.5, 10, 500);  
